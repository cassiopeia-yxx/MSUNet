...
class MUWNetModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(MUWNetModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        # pixel loss
        if self.cri_pix:
            l_pix = sum(self.cri_pix(output, self.gt) for output in self.output)
            l_total += l_pix
            loss_dict["l_pix"] = l_pix

        # perceptual loss
        if self.cri_per:
            l_per = sum(self.cri_per(output, self.gt)[0] for output in self.output)
            l_total += l_per
            loss_dict["l_per"] = l_per

        # VGG
        if self.cri_vgg:
            l_vgg = sum(self.cri_vgg(output, self.gt) for output in self.output)
            l_total += l_vgg
            loss_dict["l_vgg"] = l_vgg

        # ssim loss
        if self.cri_ssim:
            l_ssim = sum((1 - self.cri_ssim(output, self.gt)) for output in self.output)
            l_total += l_ssim
            loss_dict["l_ssim"] = l_ssim

        # fft loss
        if self.cri_fft:
            l_fft = sum(self.cri_fft(output, self.gt) for output in self.output)
            l_total += l_fft
